// PROVISIONAL — F9/F12 cost-center ("analytic distribution" — see costCentersMockFlag.ts) mock
// contract.
//
...
//     src/mocks/itemTypesHandlers.ts established) and splices each line's mocked
//     costCenterAllocations array (from src/mocks/costCentersMockFlag.ts, keyed by the line's own
//     real id) onto the real response — demonstrating the split actually "sticks" to the saved
//     line, even though it can never be sent to the real create endpoint (see
//     costCentersMockFlag.ts's header).
//
...
import { getLineDistribution, isCostCentersMockActive } from "@/mocks/costCentersMockFlag";
...
function withMockCostCenters(entry: RawEntry): RawEntry {
  ...
  return { ...entry, lines: entry.lines.map((line) => ({ ...line, costCenterAllocations: getLineDistribution(line.id) ?? [] })) };
}
